---
title: "EPPS 6323: Assignment 6"
author: "Maitreyi Pillai"
format: html
editor: visual
---
...
```{r}
library(haven)
TEDS_2016 <- read_stata("https://github.com/datageneration/home/raw/master/DataProgramming/data/TEDS_2016.dta")

# Assuming TEDS_2016 dataset is already loaded

# Step 2: Run logistic regression model with female as the sole predictor
glm.vt <- glm(votetsai ~ female, data = TEDS_2016, family = binomial)

# Step 3: Summary and plot of the model
summary(glm.vt)
plot(glm.vt)

# Step 4: Expand the model with party ID and demographic variables
glm.vt.expanded <- glm(votetsai ~ female + KMT + DPP + age + edu + income,
                       data = TEDS_2016, family = binomial)

# Step 5: Further expand the model with additional variables
glm.vt.expanded.further <- glm(votetsai ~ female + KMT + DPP + age + edu + income +
                                 Independence + Econ_worse + Govt_dont_care +
                                 Minnan_father + Mainland_father + Taiwanese,
                               data = TEDS_2016, family = binomial)
summary(glm.vt.expanded)
summary(glm.vt.expanded.further)
